...
export default function Page({ params }) {
    ...
    useEffect(() => {
        const obtenerVenta = async () => {
            try {
                ...
                const response = await obtenerTodo(`agente/obtener/venta/${external}`, token);
                console.log(response);
                const resultado = response.data;
                setventa(resultado)
                setValue('total',resultado.total);
            } catch (error) {
    ...
    const onSubmit = (data) => {
        ...
        const newData = {
            ...
            "auto": venta.auto.external_id,
        };
        console.log(newData);
        const token = getToken();

        enviar(`agente/modificar/ventas/${external}`, newData, token).then((info) => {
            if (info === '') {
                mensajes("Error en inicio de sesion", info.msg, "error");
            } else {
                console.log(info);
                mensajes("Buen trabajo!","success", "EDITADO CON ÉXITO");
    ...
    useEffect(() => {
        const updateTotalBasedOnColor = () => {
            ...
            setValue('total',venta.total+ costoColorAdicional);
        };
